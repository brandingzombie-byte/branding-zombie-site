...
import MartialArtsLeadForm from "@/components/martial-arts/MartialArtsLeadForm";
import MartialArtsJsonLd from "@/components/martial-arts/MartialArtsJsonLd";
import {
  Target,
  Lightning,
  Sparkle,
...
import {
  ...
  type MartialArtsProduct,
  type MACityCopy,
  type MAIconName,
} from "@/data/martial-arts-marketing";

const ICONS: Record<MAIconName, typeof Target> = {
  Target,
  Lightning,
  Sparkle,
  Storefront,
...
// (same picks as the tattoo hub — general trust proof, not fitness-specific).
...
export default function MartialArtsPage({
  ...
  const heroHeadline = city
    ? "Martial Arts Gym & Fitness Studio Branding in"
    : product.hero.headline;
  const heroHighlight = city ? cityLabel! : product.hero.highlight;
  const heroSubhead =
    city && cityCopy
      ? `${cityCopy.intro} Logo, brand system and an enrollment-ready website for martial arts schools and gyms — one local studio, one invoice.`
      : product.hero.subhead;

  // City pages lead the answer-first band with the local angle; the pillar
  // uses the definitional answer-first sentence (best for AI answer engines).
  ...
  const proofWork = getPortfolioItems(product.proofWorkIds);

  // Only link to sibling cities that actually have a martial-arts-branding
  // city page (others would 404, since per-city copy is hand-written).
  const siblings = (city ? getSiblingLocations(city.slug) : LOCATIONS).filter(
    (loc) => loc.slug in MARTIAL_ARTS_CITY_COPY && loc.slug !== city?.slug,
  );

  return (
    <>
      <Navbar />
      <MartialArtsJsonLd product={product} city={city} cityFaq={cityCopy?.cityFaq} />
      ...
      <main id="main-content" tabIndex={-1} className="bg-[var(--color-grave)]">
          ...
          <Reveal className="grid gap-10 lg:grid-cols-12 lg:gap-12">
            <div className="lg:col-span-7">
              <span className="text-[length:var(--text-caption)] font-semibold uppercase tracking-[0.2em] text-[var(--color-cyan-text)]">
                {isCity ? `Martial arts & gym branding in ${city!.city}` : "The short version"}
              </span>
        ...
        {/* ── 3. DUAL-AUDIENCE SEGMENTATION (dojo vocab vs. gym vocab) ── */}
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-2">
            {product.segments.map((seg, i) => (
              <Reveal
                key={seg.title}
                delay={i * 70}
                className="flex flex-col overflow-hidden rounded-2xl border border-[var(--color-dark-border-strong)] bg-[var(--color-elevated)] transition-colors hover:border-[var(--color-toxic)]/40"
              >
                <div className="relative aspect-[4/3] w-full overflow-hidden border-b border-[var(--color-dark-border)] bg-[var(--color-surface)]">
                  <Image
                    src={seg.image.src}
                    alt={seg.image.alt}
                    fill
        ...
        {/* ── 4. BENEFITS ("Why template brand kits fail gyms & dojos") ── */}
        <Section theme="dark" pad="standard" topRule>
          <Reveal>
            <SectionHeading
        ...
        {/* ── 7. THE EDGE ("Why gyms & schools pick Branding Zombie") ── */}
        <Section theme="dark" pad="standard" topRule>
        ...
        <Section theme="dark" pad="standard" topRule>
          <Reveal>
            <SectionHeading
              ...
              headline="From teardown to signed-up members,"
        ...
        {/* ── 9. REAL PORTFOLIO PROOF ── */}
        <Section theme="dark" pad="standard" topRule>
          <Reveal>
            <SectionHeading
              eyebrow={product.proofEyebrow}
              headline={product.proofHeadline}
              highlight={product.proofHighlight}
            />
            <p className="measure-wide mt-5 text-[length:var(--text-lead)] leading-relaxed text-[var(--color-dark-text-secondary)]">
              {product.proofIntro}
            </p>
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {proofWork.map((item, i) => (
              <Reveal
                key={item.id}
                delay={(i % 4) * 60}
                className="group relative flex flex-col overflow-hidden rounded-2xl border border-[var(--color-dark-border-strong)] bg-[var(--color-surface)]"
              >
                ...
                <div className="flex grow flex-col p-5">
                  {item.brand && (
                    <span className="text-[length:var(--text-caption)] font-semibold uppercase tracking-[0.14em] text-[var(--color-cyan-text)]">
                      {item.brand}
                    </span>
                  )}
                  <h3 className="mt-1.5 text-[length:var(--text-h4)] font-semibold text-[var(--color-dark-text-primary)]">
                    {item.title}
                  </h3>
                  <p className="mt-2 text-[length:var(--text-secondary)] leading-relaxed text-[var(--color-dark-text-dim)]">
                    {item.description}
        ...
        <Section theme="dark" pad="standard" topRule id="quote">
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
            <Reveal>
              ...
              <h2 className="mt-3 font-[family-name:var(--font-display)] text-[length:var(--text-h1)] leading-[1.05] tracking-tight text-[var(--color-dark-text-primary)]">
                ...
                <span className="relative inline-block">
                  mats or your membership floor
                  <span aria-hidden className="absolute -bottom-1 left-0 h-[4px] w-full bg-[var(--color-toxic)]" />
                </span>
                {isCity ? ` in ${city!.city}` : ""}.
              </h2>
              <p className="measure mt-5 text-[length:var(--text-lead)] leading-relaxed text-[var(--color-dark-text-secondary)]">
                A quick 15-minute call — we look at your logo, site and signage and name
                the 3 fastest fixes. No pitch, no pressure. Or send the form and
                we&apos;ll reply with a flat quote, usually same day.
              </p>
              <ul className="mt-6 flex flex-col gap-2.5">
                {[
                  "Free teardown — no obligation, no surprise fees",
                  "Logo, brand system, website, print & signage handled in one place",
            ...
            <Reveal delay={80}>
              <MartialArtsLeadForm source={source} ctaLabel={product.hero.ctaLabel} />
            </Reveal>
          </div>
        </Section>
        ...
        <Section theme="dark" pad="standard" topRule bottomScanline>
          <Reveal>
            <h2 className="font-[family-name:var(--font-display)] text-[length:var(--text-h3)] leading-tight text-[var(--color-dark-text-primary)]">
              {isCity
                ? `Martial arts & gym branding near ${city!.city}`
                : "Martial arts & gym branding across North Atlanta"}
            </h2>
            <p className="mt-2 text-[length:var(--text-body)] text-[var(--color-dark-text-dim)]">
              {isCity
                ? "We also build for gyms and martial arts schools in nearby towns:"
                : "Pick your town for local copy — we build for gyms and martial arts schools all over the GA-400 corridor:"}
            </p>
          </Reveal>
          {siblings.length > 0 && (
            <Reveal delay={60} className="mt-6 flex flex-wrap gap-2.5">
              {siblings.map((loc) => (
                ...
                >
                  Gym & martial arts branding in {loc.city}
                </Link>
              ))}
            </Reveal>
          )}
