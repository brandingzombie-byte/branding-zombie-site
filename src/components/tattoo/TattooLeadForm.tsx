...
import { submitTattooLead, type TattooLeadState } from "./actions";
import { trackEvent } from "@/lib/analytics";
import { cn } from "@/lib/utils";
import { ArrowUpRight, CheckCircle } from "@/components/icons";

const INITIAL: TattooLeadState = { ok: false, message: "" };
...
export default function TattooLeadForm({
  source,
  ctaLabel,
}: {
  /** Page source tag, e.g. "Tattoo Shop Marketing — Cumming, GA". */
  source: string;
  ctaLabel: string;
}) {
  const [state, formAction, pending] = useActionState(submitTattooLead, INITIAL);
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        />
        <FieldSelect
          label="New shop or established?"
          name="shopStage"
          defaultValue="Established shop"
          options={["New shop", "Established shop", "Not sure"]}
        />
      </div>
      ...
      <FieldTextarea
        ...
        rows={3}
        maxLength={4000}
        placeholder="Tell us about the shop, your style, and what you're trying to fix. A line or two is plenty."
      />
      ...
      <p className="text-[length:var(--text-caption)] text-[var(--color-dark-text-dim)]">
        Free shop teardown — no obligation. We read every request and reply within one
...
function FieldShell({
  ...
  return (
    <label htmlFor={`tat-${name}`} className="flex flex-col gap-1.5">
...
function Field({
  ...
  return (
    <FieldShell label={label} name={name} hint={hint}>
      <input id={`tat-${name}`} name={name} className={FIELD_CLASS} {...rest} />
    </FieldShell>
  );
}
...
function FieldSelect({
  ...
  return (
    <FieldShell label={label} name={name} hint={hint}>
      <select id={`tat-${name}`} name={name} className={FIELD_CLASS} {...rest}>
        {options.map((o) => (
