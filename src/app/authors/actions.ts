...
// Server action for the public author-services consult form. Mirrors the
// contact action's Resend delivery, but deliberately skips nurture-drip
// enrollment — authors aren't the SMB audience those emails target.
...
export async function submitAuthorConsult(
  ...
  if (!apiKey) {
    console.error("[authors] RESEND_API_KEY not set; form submission dropped.");
    return {
      ok: false,
      message:
  ...
  const subject = `Author lead — ${name}${book ? ` (“${book}”)` : ""} · Authors`;
  const html = `
    <h2>New author lead from brandingzombiedesigns.com/authors</h2>
    <table style="font-family: system-ui, sans-serif; font-size: 14px; border-collapse: collapse;">
      ...
      <tr><td style="padding:4px 12px;color:#666;">Source</td><td>Author services page (/authors)</td></tr>
    </table>
    ${
      message
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[authors] Resend returned ${res.status}: ${detail}`);
      return {
        ok: false,
        message:
    ...
    console.error("[authors] Resend fetch failed:", err);
    return {
      ok: false,
      message:
