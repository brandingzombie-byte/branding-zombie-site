# Print catalog — Branding Zombie Designs

Branding Zombie Designs is a full-service design + print studio in Cumming, GA (Forsyth County). We design and produce 30+ print products under one roof at wholesale pricing. Production is handled through Ink Spatter Studio, our in-house print arm. Local delivery within 30 miles of Cumming; UPS Ground elsewhere.

- **Studio**: Cumming, GA
- **Service area**: Forsyth County, North Metro Atlanta (Alpharetta, Roswell, Johns Creek, Woodstock, Buford, Suwanee, Cumming, Dawsonville)
- **Typical turnaround**: 3–10 business days for most products; 14–21 days for full booth kits
- **Pricing model**: wholesale production cost passed through plus 15–20% production fee
- **Phone**: (786) 848-1522
- **Email**: brandingzombie@gmail.com
- **Catalog page**: https://brandingzombiedesigns.com/services/print-design#print-catalog

---

## Marketing Products (8 items)

The collateral every business needs in a drawer or on a counter — designed and printed in-house so the brand stays consistent across every piece.

### Business Cards
- Starting price: $75
- Turnaround: 3–5 business days
- Options: matte, soft-touch, spot UV, foil, painted edges
- Common minimums: 100 cards

### Flyers & Sales Sheets
- Starting price: $95
- Turnaround: 3–5 business days
- Options: 4×6, 5×7, 8.5×11, gloss, matte
- Common minimums: 25 flyers

### Brochures & Trifolds
- Starting price: $180
- Turnaround: 5–7 business days
- Options: bi-fold, tri-fold, z-fold, gate-fold, gloss or matte

### Postcards & Direct Mail
- Starting price: $135
- Turnaround: 5–7 business days
- Options: EDDM 6.5×9, 4×6, 5×7, two-sided gloss
- Includes EDDM sorting and bundling for the post office

### Door Hangers
- Starting price: $110
- Turnaround: 3–5 business days
- Options: 4.25×11, 14pt gloss, perforated tear-off coupon, two-sided

### Letterhead & Envelopes
- Starting price: $140
- Turnaround: 5–7 business days
- Options: #10 envelopes, A2 cards, notepads, embossed crest

### Presentation Folders
- Starting price: $220
- Turnaround: 7–10 business days
- Options: 9×12, two-pocket, business-card slits, foil, embossed

### Business Magnets
- Starting price: $90
- Turnaround: 5–7 business days
- Options: round corner, square, die-cut shape, 30 mil

---

## Large Format & Banners (7 items)

Banners, retractable stands, posters, and outdoor signage built to survive Georgia weather and read clearly from across a parking lot.

### Retractable Banners
- Starting price: $185
- Turnaround: 5–7 business days
- Options: 33×80, 47×80, premium base, carry case included

### Outdoor Banners
- Starting price: $165
- Turnaround: 5–7 business days
- Options: 13oz vinyl, reinforced hems, grommets, custom sizes
- UV-stable inks for Georgia weather

### Indoor Banners
- Starting price: $145
- Turnaround: 5–7 business days
- Options: vinyl or fabric, grommets, pole pockets

### Mesh Banners
- Starting price: $195
- Turnaround: 5–7 business days
- Options: fence-grade mesh, reinforced edges, grommeted, custom size
- For fence wraps and high-wind installs

### Holiday & Pole Banners
- Starting price: $220
- Turnaround: 7–10 business days
- Options: seasonal sets, pole pockets, double-sided, custom size

### Banner Stands & Frames
- Starting price: $135
- Turnaround: 5–7 business days
- Options: pull-up, X-frame, L-frame, outdoor weighted
- Hardware-only kits for seasonal graphic swaps

### Posters & Outdoor Displays
- Starting price: $95
- Turnaround: 5–7 business days
- Options: 18×24, 24×36, 27×40, outdoor-rated

---

## Trade Show & Events (8 items)

Booth-ready kits — tablecloths, flags, frames, and handouts that make a 10×10 footprint look like a brand twice its size.

### Custom Tablecloths
- Starting price: $245
- Turnaround: 7–10 business days
- Options: 6-ft, 8-ft, stretch-fit, open-back, full color
- Machine washable

### Feather Flags
- Starting price: $175
- Turnaround: 7–10 business days
- Options: 9 ft, 15 ft, single or double-sided, spike or cross base

### Teardrop Flags
- Starting price: $175
- Turnaround: 7–10 business days
- Options: 9 ft, 12 ft, 15 ft, outdoor-rated

### Trade Show Booths
- Starting price: $1,450
- Turnaround: 14–21 business days
- Options: 10×10, 10×20, backlit header, counter included
- Modular kits with backdrops, monitor mounts, lit headers

### Pop-Up Displays & Floor Graphics
- Starting price: $945
- Turnaround: 10–14 business days
- Options: curved pop-up, floor decals, carpet tiles, carry case

### SEG Display Frames
- Starting price: $385
- Turnaround: 10–14 business days
- Options: wall mount, free-standing, backlit, modular
- Silicone-edge graphic frames with tensioned fabric prints

### Trade Show Handouts
- Starting price: $180
- Turnaround: 5–7 business days
- Options: bundled kits, rack cards, sample packs, branded sleeves

### Promo Giveaways
- Starting price: $215
- Turnaround: 10–14 business days
- Options: drinkware, totes, tech accessories, apparel add-ons

---

## Sample bundles (real all-in pricing)

### Cumming barbershop starter kit — $620 all-in, 7 days
- 500 business cards (soft-touch)
- 1 retractable banner (33×80)
- 100 door hangers (perforated coupon)

### Forsyth County food-truck event kit — $815 all-in, 10 days
- Custom tablecloth (6-ft, stretch-fit)
- Teardrop flag (12 ft)
- 250 menu flyers (4×6, two-sided)

### Trade-show starter (10×10 booth) — $1,940 all-in, 14 days
- Pop-up display backdrop
- 6-ft printed tablecloth
- 500 rack-card handouts

---

## Comparison: one shop vs. designer + print shop

| Topic | Branding Zombie (one shop) | Designer + print shop |
| --- | --- | --- |
| Who you call | One studio, one project manager, one invoice | Designer + print rep + sometimes an installer — three threads |
| Print-ready files | Designed to the press specs; runs clean the first time | Handoff → printer rejects → edits ferried back |
| Pricing | Wholesale pricing passed through; one quote for both halves | Retail markup on both design AND printing |
| Turnaround | Most jobs ship in 3–10 days; rush options available | Each handoff adds a queue — 2 weeks becomes 4 |

---

## Custom requests not in the catalog

We also handle yard signs, vehicle wraps, custom apparel (screen print, DTF, embroidery), product packaging, labels, and dielines. Send a description of what you need to brandingzombie@gmail.com or book a 15-minute discovery call at https://calendly.com/brandingzombie/discovery for a real quote — usually back within one business day.

_Last updated: 2026-06-02._
